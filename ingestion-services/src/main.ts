...
async function bootstrap() {
  const app = await NestFactory.createMicroservice<MicroserviceOptions>(
    AppModule,
    {
      ...
      options: {
        ...
        port: Number(process.env.INGESTION_SERVICE_PORT),
      },
      logger: new ConsoleLogger({
        colors: true,
        prefix: 'MicroService-Ingestion',
      }),
    },
  );
  ...
  await app.listen();
  console.log(`Ingestion Service started host:- http://${process.env.INGESTION_SERVICE_HOST}:${process.env.INGESTION_SERVICE_PORT}`)
